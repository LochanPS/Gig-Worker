...
export interface Payment {
  ...
  // The settlement wallets the escrow moved value between. Shown on the payment
  // page so a payout is traceable to the accounts it actually touched, rather than
  // only naming the network it ran on.
...
export interface CustomerSummary {
  ...
  verified: boolean;
  walletAddress: string | null;
  // True when the platform holds a key for this wallet, so it can sign FROM the
  // account. A party created with an address but no key is receive-only: it can
  // be paid, but cannot fund a payment of its own.
  canSign: boolean;
  // Whether the wallet was supplied by an operator or generated by the platform —
  // the UI says which, so a generated demo address is never mistaken for a real one.
  walletSource: 'PROVIDED' | 'GENERATED' | null;
  // The payout destination the off-ramp will push to, if any. Null means a payment
  // to this party settles on-chain and then lands in PAYOUT_FAILED.
  payoutDestination: { method: PayoutMethod; currency: Currency; masked: string } | null;
  createdAt: string;
  paymentsCount?: number;
}

// What the backend is actually doing when it "settles" — the one fact the UI had
// no way to know. In simulated mode every tx hash is a random string; in real mode
// they are chain transactions you can open on an explorer.
export interface SystemInfo {
  settlementMode: 'real' | 'simulated';
  // Set when SETTLEMENT_MODE=real was requested but the chain was unreachable at
  // boot and the backend fell back to simulated (ROADMAP risk R1).
  degraded: boolean;
  chainId: number;
  chainName: string;
  explorerUrl: string | null;
  contracts: Record<string, string> | null; // deployed addresses, when on-chain
}
...
export interface FreelancerSummary {
  ...
  // The destination the off-ramp would push to, so the wizard can show where the
  // money lands before the company commits, not only after it has landed.
  payoutDestination: { method: PayoutMethod; currency: Currency; masked: string } | null;
}
